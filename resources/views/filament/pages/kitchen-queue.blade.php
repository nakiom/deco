    ...
    <div class="space-y-6">
        <div class="flex flex-wrap items-center justify-between gap-3">
            <p class="text-sm text-gray-600 dark:text-gray-400">
                Deslizá cada fila hacia la derecha para marcar el plato como listo. Podés deshacer la última acción si te equivocás.
            </p>
            <button
                type="button"
                wire:click="undoLast"
        ...
        <div class="space-y-3">
            @foreach ($this->getKitchenItems() as $item)
                <div
                    wire:key="kitchen-item-{{ $item->id }}"
                    x-data="{
                        startX: 0,
                        startY: 0,
                        x: 0,
        ...
        @if($this->getKitchenItems()->isEmpty())
            <div class="rounded-xl border border-dashed border-gray-300 py-14 text-center text-gray-500 dark:border-gray-600 dark:text-gray-400">
                <x-heroicon-o-fire class="mx-auto mb-3 h-14 w-14 opacity-40" />
                <p class="text-lg font-medium">No hay pedidos en cocina</p>
            </div>
        @endif
        ...
        <div class="border-t border-gray-200 pt-6 dark:border-gray-700">
            ...
            <ul class="space-y-2 text-sm">
                @forelse ($this->getRecentHistory() as $action)
                ...
                @empty
                    <li class="text-gray-500 dark:text-gray-400">Todavía no hay platos marcados en esta sesión.</li>
                @endforelse
            </ul>
        </div>
    </div>
