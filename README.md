...
- **Pedidos desde la carta (QR):** en **Mesas** o **Mapa operativo → cuenta de mesa**, activá **Clientes pueden pedir desde la carta (QR)**. En la carta, **Agregar al pedido** arma un carrito local; podés **quitar** líneas antes de **Enviar pedido a cocina** (un solo POST con todas las líneas). Nombre para la cuenta, ítem **individual** o **compartido en partes iguales**. Los ítems pasan al pedido abierto de la mesa (`source: qr_table`). En el panel de cobro: **resumen por persona** (estimado).
